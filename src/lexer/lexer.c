...
char lexer_peek(lexer_t *lexer) { return *(lexer->current); }
...
token_t lexer_next_token(lexer_t *lexer) {
  ...
  default:
    if (isdigit(c)) {
      ...
      return (token_t){has_dot ? TOKEN_FLOAT : TOKEN_INT, strdup(buffer), i,
                       line};
    }
...
void token_free(token_t *token) {
  ...
  if (token->type == TOKEN_INT || token->type == TOKEN_FLOAT ||
      token->type == TOKEN_STRING || token->type == TOKEN_IDENTIFIER) {
