...
void mark(vm_t *vm)
{
    ...
    for (size_t i = 0; i < vm->frames->count; i++)
    {
        ...
        for (size_t j = 0; j < frame->references->count; j++)
        {
            ...
            if (!obj)
            {
                fprintf(stderr, "[WARNING] Null object reference at frame %zu, index %zu\n", i, j);
                continue;
            }

            if ((uintptr_t)obj < 0x1000 || (uintptr_t)obj > 0x7fffffffffff)
            { // Check for valid address range
                fprintf(stderr, "[ERROR] Detected invalid object pointer at frame %zu, index %zu: %p\n", i, j, obj);
                continue;
            }

            if (obj->is_marked)
            {
                fprintf(stderr, "[WARNING] Object already marked at frame %zu, index %zu\n", i, j);
                continue;
            }
