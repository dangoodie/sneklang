...
MunitResult test_lexer_full_script(const MunitParameter params[],
  ...
  token_type_t expected_types[] = {
      ...
      TOKEN_FLOAT,      TOKEN_EOF,        TOKEN_IDENTIFIER, TOKEN_EQUAL,
      TOKEN_STRING,     TOKEN_EOF,        TOKEN_IDENTIFIER, TOKEN_EQUAL,
      TOKEN_LBRACKET,   TOKEN_FLOAT,      TOKEN_COMMA,      TOKEN_FLOAT,
      TOKEN_COMMA,      TOKEN_FLOAT,      TOKEN_RBRACKET,   TOKEN_EOF,
      TOKEN_IDENTIFIER, TOKEN_GREATER,    TOKEN_INT,        TOKEN_IDENTIFIER,
      TOKEN_LPAREN,     TOKEN_IDENTIFIER, TOKEN_RPAREN,     TOKEN_EOF,
      TOKEN_EOF};
  ...
  for (int i = 0; i < expected_count; i++) {
    printf("Token %d: Got type %d, Expected type %d, Lexeme: '%s'\n", i,
           tokens[i]->type, expected_types[i], tokens[i]->lexeme);
  }
